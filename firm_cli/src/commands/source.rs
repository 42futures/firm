...
pub fn find_entity_source(
    ...
    let source_path = if entity_type == "schema" {
        workspace.find_schema_source(&entity_id)
    } else {
        workspace.find_entity_source(&entity_type, &entity_id)
    };
    ...
    match source_path {
        ...
        None => {
            let error_msg = if entity_type == "schema" {
                format!("Schema with name '{}' not found in workspace", entity_id)
            } else {
                format!(
